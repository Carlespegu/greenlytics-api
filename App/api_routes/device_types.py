...
def search_device_types(payload: DeviceTypeSearchRequest, db: Session = Depends(get_db)):
    return search_device_types_service(db, payload)


@router.get("/{device_type_id}", response_model=DeviceTypeResponse)
def get_device_type(device_type_id: UUID, db: Session = Depends(get_db)):
...
def create_device_type(payload: DeviceTypeCreate, db: Session = Depends(get_db)):
    return create_device_type_service(db, payload)


@router.put("/{device_type_id}", response_model=DeviceTypeResponse)
def update_device_type(device_type_id: UUID, payload: DeviceTypeUpdate, db: Session = Depends(get_db)):
...
def delete_device_type(device_type_id: UUID, db: Session = Depends(get_db)):
